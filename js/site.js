(() => {
  ...
  function renderContact(){
    ...
    $('#contactForm')?.addEventListener('submit',async e=>{e.preventDefault(); const btn=e.currentTarget.querySelector('button'); const old=btn.textContent; btn.disabled=true; btn.textContent=state.lang==='az'?'Göndərilir...':'Sending...'; try{const body=Object.fromEntries(new FormData(e.currentTarget)); const r=await fetch('/api/contact',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}); const data=await r.json().catch(()=>({})); if(!r.ok)throw new Error(data.error||(state.lang==='az'?'Mesaj göndərilmədi.':'Message could not be sent.')); e.currentTarget.reset(); alert(state.lang==='az'?'Mesaj göndərildi.':'Message sent.')}catch(err){alert(err?.message||(state.lang==='az'?'Mesaj göndərilmədi.':'Message could not be sent.'))} finally{btn.disabled=false;btn.textContent=old}})
  }
